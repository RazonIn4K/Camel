...
class TestRateLimiter(unittest.TestCase):
    ...
    async def test_rate_limiter_jitter(self):
        ...
        rate_limiter_with_jitter = RateLimiter(requests_per_minute=60, burst_size=1, jitter=True)
        ...
        rate_limiter_without_jitter = RateLimiter(requests_per_minute=60, burst_size=1, jitter=False)
...
class TestConvenienceFunctions(unittest.TestCase):
    ...
    async def test_run_parallel_tasks_with_kwargs(self):
        ...
        results, metrics = await run_parallel_tasks_with_kwargs(task_func, items_with_kwargs, max_workers=2)
    ...
    def test_run_parallel_with_kwargs_sync(self):
        ...
        # Run tasks in parallel with kwargs
        results, metrics = run_parallel_with_kwargs_sync(task_func, items_with_kwargs, max_workers=2)
...
class TestGraySwanIntegration(unittest.TestCase):
    ...
    async def test_run_parallel_reconnaissance(self, mock_recon_agent):
        ...
        from cybersec_agents.grayswan.utils.parallel_processing import run_parallel_reconnaissance
    ...
    async def test_run_parallel_prompt_engineering(self, mock_prompt_agent):
        ...
        from cybersec_agents.grayswan.utils.parallel_processing import run_parallel_prompt_engineering
        ...
        mock_instance.generate_prompts_of_type.side_effect = lambda **kwargs: ["prompt1", "prompt2"]
    ...
    @patch('cybersec_agents.grayswan.agents.exploit_delivery_agent.ExploitDeliveryAgent')
    async def test_run_parallel_exploits(self, mock_exploit_agent):
        """Test that run_parallel_exploits works correctly."""
        from cybersec_agents.grayswan.utils.parallel_processing import run_parallel_exploits
        ...
        mock_instance._execute_via_api.side_effect = lambda prompt, model: f"Response to {prompt}"
        
        # Mock the _analyze_response method
        mock_instance._analyze_response.side_effect = lambda response, behavior: (True, "Success")
    ...
    async def test_run_parallel_evaluation(self, mock_eval_agent):
        ...
        from cybersec_agents.grayswan.utils.parallel_processing import run_parallel_evaluation
