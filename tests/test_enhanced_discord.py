...
class TestEnhancedDiscordScraper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.search_patcher = patch(
            "cybersec_agents.grayswan.utils.discord_utils.DiscordScraper.search"
        )
        ...
        self.mock_sia.polarity_scores.side_effect = lambda text: {
            "neg": 0.1
            if "trouble" in text or "hate" in text or "frustrating" in text
            else 0.0,
            "neu": 0.5 if "neutral" in text else 0.3,
            "pos": 0.8
            if "amazing" in text
            or "better" in text
            or "working well" in text
            or "safer" in text
            else 0.0,
            "compound": 0.8
            if "amazing" in text
    ...
    def test_search_with_filters_date_filter(self):
        ...
        results: list[Any] = self.scraper.search_with_filters(
            "test query", min_date="2023-06-16", max_date="2023-06-18"
        )
    ...
    def test_search_with_filters_author_filter(self):
        ...
        results: list[Any] = self.scraper.search_with_filters(
            "test query", author_filter="User1"
        )
    ...
    def test_search_with_filters_content_filter(self):
        ...
        results: list[Any] = self.scraper.search_with_filters(
            "test query", content_filter=r"jailbreak|alignment"
        )
    ...
    def test_search_with_filters_has_attachments(self):
        ...
        results: list[Any] = self.scraper.search_with_filters(
            "test query", has_attachments=True
        )
    ...
    def test_search_with_filters_sentiment_filter(self):
        ...
        results: list[Any] = self.scraper.search_with_filters(
            "test query", sentiment_filter="positive", include_sentiment=True
        )
    ...
    def test_search_with_filters_include_sentiment(self):
        ...
        results: list[Any] = self.scraper.search_with_filters(
            "test query", include_sentiment=True
        )
    ...
    def test_get_trending_topics(self):
        ...
        with patch(
            "cybersec_agents.grayswan.utils.enhanced_discord_utils.word_tokenize"
        ) as mock_tokenize, patch(
            "cybersec_agents.grayswan.utils.enhanced_discord_utils.stopwords"
        ) as mock_stopwords, patch(
            "cybersec_agents.grayswan.utils.enhanced_discord_utils.Counter"
        ) as mock_counter:
            # Set up the mocks
    ...
    def test_get_time_distribution(self):
        ...
        day_dist = self.scraper.get_time_distribution(
            self.sample_results, interval="day"
        )
        weekday_dist = self.scraper.get_time_distribution(
            self.sample_results, interval="weekday"
        )
        ...
        self.assertEqual(
            len(weekday_dist), 5
        )  # Might be less if some messages are on the same weekday

    def test_search_by_timeframe(self):
        """Test search_by_timeframe."""
        # Mock datetime.now to return a fixed date
        with patch(
            "cybersec_agents.grayswan.utils.enhanced_discord_utils.datetime"
        ) as mock_datetime:
            # Set up the mock
            ...
            this_week_results = self.scraper.search_by_timeframe(
                "test query", "this_week"
            )
    ...
    def test_load_results(self, mock_os, mock_json):
        ...
        with patch("builtins.open", mock_open):
            ...
            mock_os.path.join.assert_called_with(
                "data", "discord_searches", "test_results.json"
            )
            mock_os.path.exists.assert_called_once()
            mock_open.assert_called_once_with(
                "/".join(["data", "discord_searches", "test_results.json"]),
                "r",
                encoding="utf-8",
            )
