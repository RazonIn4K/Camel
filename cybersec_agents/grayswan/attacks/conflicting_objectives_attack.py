"""Conflicting Objectives Attack for Gray Swan Arena.

This attack attempts to make the target AI agent pursue conflicting objectives
simultaneously. It leverages the PromptEngineerAgent for crafting prompts and
the ExploitDeliveryAgent for executing the attack.
"""
...
class ConflictingObjectivesAttack(Attack):
    """Class to orchestrate the conflicting objectives attack."""
    ...
    def gather_intelligence(self) -> Dict[str, Any]:
        ...
        """
        logger.info("No intelligence gathering required for ConflictingObjectivesAttack")
    ...
    def craft_prompts(self, intelligence: Dict[str, Any]) -> List[str]:
        ...
        challenge_details = self.challenge_loader.get_challenge_details(
            ...
            challenge_name="Make Agent Pursue Conflicting Objectives",
        )

        # Generate prompts using the gathered intelligence and challenge details
        prompts = self.prompt_engineer.generate_prompts(
            target_model="conflicting_objectives",
            target_behavior="pursue_conflicting_objectives",
