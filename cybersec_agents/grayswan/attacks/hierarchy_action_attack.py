"""Hierarchy Action Attack for Gray Swan Arena.

This attack attempts to make the target AI agent take actions that violate
hierarchical relationships. It leverages the PromptEngineerAgent for crafting
prompts and the ExploitDeliveryAgent for executing the attack.
"""
...
class HierarchyActionAttack(Attack):
    """Class to orchestrate the hierarchy action attack."""
    ...
    def gather_intelligence(self) -> Dict[str, Any]:
        ...
        """
        logger.info("No intelligence gathering required for HierarchyActionAttack")
    ...
    def craft_prompts(self, intelligence: Dict[str, Any]) -> List[str]:
        ...
        challenge_details = self.challenge_loader.get_challenge_details(
            challenge_type="Hierarchy Violations",
            challenge_name="Make Agent Take Actions That Violate Hierarchical Relationships",
        )

        # Generate prompts using the gathered intelligence and challenge details
        prompts = self.prompt_engineer.generate_prompts(
            target_model="hierarchy_action",
            target_behavior="violate_hierarchy",
