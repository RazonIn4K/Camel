...
def advanced_search_example():
    ...
    one_month_ago: tuple[Any, ...] = (
        (datetime.now().replace(day=1) - datetime.timedelta(days=1))
        .replace(day=1)
        .strftime("%Y-%m-%d")
    )
    ...
    print(
        scraper.format_advanced_results(
            results, include_metadata=True, include_stats=True, max_messages=5
        )
    )
...
def sentiment_analysis_example():
    ...
    results: list[Any] = scraper.search_with_filters(
        query=query, limit=30, include_sentiment=True
    )
    ...
    for sentiment in ["positive", "negative", "neutral"]:
        ...
        for result in results:
            if (
                "sentiment" in result
                and result["sentiment"]["classification"] == sentiment
            ):
                print(
                    f"From {result['author']} (score: {result['sentiment']['compound']:.2f}):"
                )
...
def timeframe_search_example():
    ...
    for timeframe in timeframes:
        ...
        results: list[Any] = scraper.search_by_timeframe(
            query=query, timeframe=timeframe, limit=20
        )
