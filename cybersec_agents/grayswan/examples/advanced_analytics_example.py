...
def basic_feature_analysis_example(results: List[Dict[str, Any]], output_dir: str):
    ...
    print(
        f"Extracted {len(features_df)} samples with {len(features_df.columns)} features"
    )
    print("\nFeature columns:")
    ...
    if kmeans_results:
        ...
        if analysis_file and os.path.exists(analysis_file):
            ...
            for cluster in analysis.get("clusters", []):
                print(
                    f"- Cluster {cluster.get('cluster_id', '?')}: {cluster.get('size', 0)} samples, "
                ...
                if common_models:
                    ...
                    for model, count in sorted(
                        common_models.items(), key=lambda x: x[1], reverse=True
                    )[:3]:
                        print(f"  - {model}: {count} samples")

    # Create DBSCAN clustering
    dbscan_results = create_advanced_clustering(results, output_dir, algorithm="dbscan")
