...
def generate_sample_data(num_samples: int = 50) -> List[Dict[str, Any]]:
    ...
    for i in range(num_samples):
        ...
        model = random.choices(models, weights=[0.3, 0.3, 0.2, 0.1, 0.1], k=1)[0]
        ...
        prompt = (
            f"Sample prompt {i+1} using {prompt_type} approach with {attack_vector} technique. "
            + "X" * (prompt_length - 50)
        )
        ...
        result: Any = {
            ...
            "has_code_markers": random.random()
            < 0.3,  # 30% chance of having code markers
...
def main():
    ...
    parser.add_argument(
        "--load-data", type=str, help="Load data from a file instead of generating"
    )
    parser.add_argument(
    ...
    attack_pattern_file = create_attack_pattern_visualization(
        results, args.output_dir, title="Sample Attack Pattern Clustering"
    )
    ...
    similarity_network_file = create_prompt_similarity_network(
        results, args.output_dir, title="Sample Prompt Similarity Network"
    )
    ...
    prediction_file, prediction_metrics = create_success_prediction_model(
        results, args.output_dir, title="Sample Success Prediction Model"
    )
    if prediction_file:
    ...
    if args.interactive:
        ...
        dashboard_file = create_interactive_dashboard(
            results, args.output_dir, title="Sample Gray Swan Arena Dashboard"
        )
        if dashboard_file:
    ...
    report_files = create_advanced_evaluation_report(
        results, args.output_dir, include_interactive=args.interactive
    )
    ...
    if args.interactive and "interactive_dashboard" in report_files:
        ...
        logger.info(
            "  Or run: python -m http.server --directory {0} 8000".format(
                os.path.dirname(dashboard_path)
            )
        )
        logger.info(
            "  Then navigate to http://localhost:8000/{0}".format(
