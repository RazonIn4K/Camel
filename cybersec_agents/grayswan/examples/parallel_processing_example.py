...
async def parallel_kwargs_example():
    ...
    # Create task manager
    task_manager = TaskManager(
        max_workers=3, requests_per_minute=30, burst_size=3, max_retries=1
    )
...
def decorator_example():
    ...
    def process_data(item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Process the item
        return {"id": item["id"], "value": item["value"] * 2, "processed": True}
...
async def simulate_gray_swan_pipeline():
    ...
    async def mock_discord_search(query: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "results": [
                {
                    "author": "User1",
                    "content": f"Discussing {query}",
                    "timestamp": datetime.now().isoformat(),
                },
                {
                    "author": "User2",
                    "content": f"Replying about {query}",
                    "timestamp": datetime.now().isoformat(),
                },
            ],
            "timestamp": datetime.now().isoformat(),
    ...
    # Create task manager
    task_manager = TaskManager(
        max_workers=2, requests_per_minute=30, burst_size=2, max_retries=1
    )
    ...
    tasks: list[Any] = [
        ...
        task_manager.execute_task(
            mock_discord_search, f"{target_model} {target_behavior}"
        ),
    ]
    ...
    print(
        f"Executed {len(exploit_results)} prompts and saved results to {results_path}"
    )
    print(
        f"Success rate: {sum(1 for r in exploit_results if r['success']) / len(exploit_results):.1%}"
    )
