...
class GraySwanPipeline:
    ...
    async def run_parallel_reconnaissance(
        ...
        self.logger.info(
            f"Parallel reconnaissance completed, report saved to {report_path}"
        )
    ...
    def run_reconnaissance(
        ...
        """
        self.logger.info(
            f"Starting reconnaissance phase for {target_model} - {target_behavior}"
        )
    ...
    def run_evaluation(
        ...
        """
        self.logger.info(
            f"Starting evaluation phase for {target_model} - {target_behavior}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Evaluation phase completed, results saved to {eval_path}"
            )
    ...
    async def run_full_pipeline_async(
        ...
        skip_phases_list: List[str] = skip_phases if skip_phases is not None else []
        ...
        try:
            ...
            if "recon" not in skip_phases_list:
            ...
            if "prompt" not in skip_phases_list:
                self.logger.info("Starting prompt engineering phase")
                recon_report = {}
                if isinstance(results, dict) and "reconnaissance" in results:
                    recon_data = results.get("reconnaissance", {})
                    recon_report = recon_data.get("report", {})
            ...
            if "exploit" not in skip_phases_list and isinstance(results, dict) and "prompt_engineering" in results:
                self.logger.info("Starting exploit delivery phase")
                prompts = []
                if "prompts" in results["prompt_engineering"]:
            ...
            # Phase 4: Evaluation
            if "eval" not in skip_phases_list and isinstance(results, dict) and "exploit_delivery" in results:
                self.logger.info("Starting evaluation phase")
                exploit_results_list = []
                if "results" in results["exploit_delivery"]:
    ...
    def run_full_pipeline(
        ...
        """
        Run the complete Gray Swan Arena pipeline synchronously.
        ...
        try:
            ...
            if "exploit" not in skip_phases_list and isinstance(results, dict) and "prompt_engineering" in results:
                ...
                exploit_results = asyncio.run(
                    self.run_parallel_exploits(
                        prompts=prompts,
                        target_model=target_model,
                        target_behavior=target_behavior,
                        method=test_method,
            ...
            if "eval" not in skip_phases_list and isinstance(results, dict) and "exploit_delivery" in results:
                self.logger.info("Starting evaluation phase")
                eval_results = self.run_evaluation(
                    exploit_results=results["exploit_delivery"]["results"],
                    target_model=target_model,
                    target_behavior=target_behavior,
                    include_advanced_visualizations=include_advanced_visualizations,
                    include_interactive_dashboard=include_interactive_dashboard,
...
def main():
    ...
    config_dict: dict[str, Any] = {
        ...
        "agents": {
            ...
            "exploit_delivery": {
                ...
                "model_name": args.agent_model,
                "browser_method": args.test_method
                if args.test_method == "web"
                else "playwright",
                "headless": True,
            },
            "evaluation": {
    ...
    if args.legacy:
        ...
        # Run full pipeline
        results: dict[str, Any] = pipeline.run_full_pipeline(
            target_model=args.target_model,
            target_behavior=args.target_behavior,
        ...
        if "error" in results:
            ...
            if args.interactive_dashboard and "evaluation" in results:
                ...
                dashboard_path = next(
                    (p for k, p in vis_paths.items() if "dashboard" in k.lower()), None
                )
                if dashboard_path:
                    print(
