...
from .browser_utils import BrowserDriver, PlaywrightDriver, SeleniumDriver, BrowserMethod
...
class EnhancedPlaywrightDriver(PlaywrightDriver):
    ...
    def __init__(self, headless: bool = True, retry_attempts: int = 3, retry_delay: float = 1.0):
        """
    ...
    def initialize(self):
        ...
        return self.with_retry(
            _initialize, 
            "Error initializing enhanced Playwright browser"
        )
        ...
        try:
            ...
            def enter_prompt(selector):
                ...
                if "contenteditable" in selector:
                    self.page.evaluate(f'document.querySelector("{selector}").innerText = "{prompt}"')
                else:
