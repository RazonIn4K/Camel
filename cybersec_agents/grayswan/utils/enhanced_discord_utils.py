...
try:
    ...
    if all(nltk_status.values()):
        ...
        if NLP_AVAILABLE:
            ...
            logger.warning("Sentiment analyzer not available. Some features will be limited.")
    else:
        NLP_AVAILABLE = False
        missing_packages = [pkg for pkg, status in nltk_status.items() if not status]
        logger.warning(f"NLTK initialization incomplete. Missing packages: {', '.join(missing_packages)}")
except ImportError:
    NLP_AVAILABLE = False
    ...
    def search_with_filters(
        ...
        if cache_key in self.result_cache:
            ...
            if time.time() - cache_entry["timestamp"] < self.cache_expiry:
                ...
                if sentiment_filter and self.nlp_available:
                    results = self._filter_by_sentiment(results, sentiment_filter, include_sentiment)
        ...
        if min_date:
            try:
                ...
                results = [
                    r for r in results 
                    if datetime.fromisoformat(r["timestamp"].split("T")[0]) >= min_datetime
                ]
            except ValueError:
        ...
        if max_date:
            try:
                ...
                results = [
                    r for r in results 
                    if datetime.fromisoformat(r["timestamp"].split("T")[0]) <= max_datetime
                ]
            except ValueError:
                logger.warning(f"Invalid max_date format: {max_date}. Expected YYYY-MM-DD")
        
        if author_filter:
            author_filter_lower = author_filter.lower()
            results = [
                r for r in results 
                if author_filter_lower in r["author"].lower() or author_filter_lower in r["author_id"].lower()
            ]
        ...
        if has_attachments is not None:
            results = [
                r for r in results 
                if (len(r["attachments"]) > 0) == has_attachments
            ]
        
        if has_mentions is not None:
            results = [
                r for r in results 
                if (len(r["mentions"]) > 0) == has_mentions
            ]
...
if __name__ == "__main__":
    ...
    if discord_scraper.available:
        ...
        print(discord_scraper.format_advanced_results(
            results,
