...
def create_feature_distribution_plots(
    ...
    try:
        ...
        for i in range(0, len(features), 4):
            ...
            # Save plot
            output_file = os.path.join(dist_dir, f"feature_distributions_batch_{i//4 + 1}.png")
            plt.savefig(output_file, dpi=300, bbox_inches="tight")
            plt.close()
    ...
    try:
        ...
        if features is None:
            ...
            if hue in df.columns and df[hue].nunique() <= 5:
                ...
                features = corr_with_target.index[1:max_features+1].tolist()  # Skip the target itself
            else:
...
def create_advanced_clustering(
    ...
    try:
        ...
        if len(df) > 50:
            ...
            tsne = TSNE(n_components=2, perplexity=min(perplexity, len(df)-1), random_state=random_state)
        ...
        if algorithm.lower() == "kmeans":
            ...
            legend_elements = [
                plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=plt.cm.viridis(i/n_clusters), 
            ...
            for i, cluster_id in enumerate(unique_clusters):
                if cluster_id == -1:
                    ...
                    legend_elements.append(
                        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=plt.cm.viridis(color_idx), 
                                  markersize=10, label=f'Cluster {cluster_id+1}')
                    )
            ...
            for cluster_id in sorted(df["cluster"].unique()):
                ...
                for col in ["model", "prompt_type", "attack_vector"]:
                    if col in cluster_df.columns:
                        cluster_info[f"common_{col}s"] = cluster_df[col].value_counts().to_dict()
                
                cluster_analysis["clusters"].append(cluster_info)
            
            # Save cluster analysis
            analysis_file = os.path.join(output_dir, "advanced_cluster_analysis_dbscan.json")
            with open(analysis_file, "w") as f:
                json.dump(cluster_analysis, f, indent=2)
...
def analyze_clusters(
    ...
    for i in range(kmeans.n_clusters):
        ...
        cluster_info = {
            "cluster_id": i,
            "size": len(cluster_df),
            "success_rate": float(cluster_df["success"].mean()) if "success" in cluster_df.columns else None,
        }
        ...
        for col in feature_cols:
            ...
            if global_std > 0:
                distances[col] = float(abs(cluster_means[col] - global_means[col]) / global_std)
            else:
                distances[col] = 0.0
...
def create_advanced_model_comparison(
    ...
    try:
        ...
        prompt_types = [col for col in model_metrics.columns if col not in ["Model", "Success_mean", "Success_count", "Response Time_mean", "Response Time_median", "Response Time_std"]]
            ...
            for metric in radar_metrics:
                if metric == "Response Time_mean":
                    ...
                    if max_time > 0:
                        ...
                        value = 1 - (model_data["Response Time_mean"].values[0] / max_time)
                    else:
                        value = 1.0
                else:
        ...
        for col in model_table.columns:
            if col != "Model" and model_table[col].dtype in [np.float64, np.float32]:
                if "Time" in col:
                    ...
                    # Format percentages
                    model_table[col] = (model_table[col] * 100).round(1).astype(str) + "%"
...
def create_advanced_success_prediction_model(
    ...
    try:
        ...
        feature_cols = [col for col in df.columns if col not in ["success", "model", "prompt_type", "attack_vector"]]
        ...
        # Customize plot
        plt.title(f"Advanced Success Prediction - Feature Importance ({model_type.replace('_', ' ').title()})", fontsize=16)
        plt.xlabel("Importance", fontsize=14)
        plt.ylabel("Feature", fontsize=14)
        plt.tight_layout()
        
        # Save plot
        importance_file = os.path.join(output_dir, f"advanced_success_prediction_importance_{model_type}.png")
        ...
        # Save plot
        roc_file = os.path.join(output_dir, f"advanced_success_prediction_roc_{model_type}.png")
        ...
        # Save plot
        cm_file = os.path.join(output_dir, f"advanced_success_prediction_cm_{model_type}.png")
        ...
        metrics = {
            ...
            "feature_importance": dict(zip(importance_df["Feature"].tolist(), importance_df["Importance"].tolist())),
        ...
        metrics_file = os.path.join(output_dir, f"advanced_success_prediction_metrics_{model_type}.json")
...
def create_advanced_analytics_report(
    ...
    if include_clustering:
        ...
        if clustering_results:
            report_files["advanced_clustering"] = clustering_results.get("clustering_file", "")
            report_files["cluster_analysis"] = clustering_results.get("analysis_file", "")
