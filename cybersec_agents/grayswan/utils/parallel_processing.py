...
class RateLimiter:
    ...
    def __init__(
        self, requests_per_minute: int = 60, burst_size: int = 10, jitter: bool = True
    ):
        """
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["tasks_submitted"] > 0:
            metrics["success_rate"] = (
                metrics["tasks_completed"] / metrics["tasks_submitted"]
            )
        else:
...
def parallel_map(
    ...
    def decorator(func: Callable) -> Callable:
        @wraps(func)
        def wrapper(
            items: List[Any], *args: Any, **kwargs: Any
...
async def run_parallel_reconnaissance(
    ...
    logger.info(
        f"Starting enhanced parallel reconnaissance for {target_model} - {target_behavior}"
    )
    ...
    if include_web_search:
        tasks.append(
            asyncio.create_task(
                asyncio.to_thread(
                    recon_agent.run_web_search, target_model, target_behavior
                )
            )
        )
    ...
    if include_discord_search:
        tasks.append(
            asyncio.create_task(
                asyncio.to_thread(
                    recon_agent.run_discord_search, target_model, target_behavior
                )
            )
        )
    ...
    if include_github_search and hasattr(recon_agent, "run_github_search"):
        tasks.append(
            asyncio.create_task(
                asyncio.to_thread(
                    recon_agent.run_github_search, target_model, target_behavior
                )
            )
        )
    ...
    if include_twitter_search and hasattr(recon_agent, "run_twitter_search"):
        tasks.append(
            asyncio.create_task(
                asyncio.to_thread(
                    recon_agent.run_twitter_search, target_model, target_behavior
                )
            )
        )
    ...
    logger.info(
        f"Enhanced parallel reconnaissance completed, report saved to {report_path}"
    )
...
async def run_parallel_prompt_engineering(
    ...
    """
    from cybersec_agents.grayswan.agents.prompt_engineer_agent import (
        PromptEngineerAgent,
    )

    logger.info(
        f"Starting parallel prompt engineering for {target_model} - {target_behavior}"
    )
    ...
    logger.info(
        f"Parallel prompt engineering completed, {len(all_prompts)} prompts saved to {prompts_path}"
    )
...
async def run_parallel_exploits(
    ...
    """
    from cybersec_agents.grayswan.agents.exploit_delivery_agent import (
        ExploitDeliveryAgent,
    )

    logger.info(
        f"Starting enhanced parallel exploit delivery for {target_model} with {len(prompts)} prompts"
    )
        ...
        try:
            ...
            if method == "api":
                response = await asyncio.to_thread(
                    exploit_agent._execute_via_api, prompt, target_model
                )
            elif method == "web":
                response = await asyncio.to_thread(
                    exploit_agent._execute_via_web, prompt, target_model
                )
            else:
                response = await asyncio.to_thread(
                    exploit_agent._execute_via_api, prompt, target_model
            ...
            success, reason = await asyncio.to_thread(
                exploit_agent._analyze_response, response, target_behavior
            )
    ...
    logger.info(
        f"Enhanced parallel exploit delivery completed, results saved to {results_path}"
    )
...
async def run_parallel_evaluation(
    ...
    if include_visualizations:
        ...
        vis_paths = await asyncio.to_thread(
            eval_agent.create_visualizations, evaluation, target_model, target_behavior
        )
    ...
    summary = await asyncio.to_thread(
        eval_agent.generate_summary, evaluation, target_model, target_behavior
    )
