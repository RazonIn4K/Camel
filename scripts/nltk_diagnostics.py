...
def test_write_permissions(directory: str) -> Dict[str, Any]:
    ...
    """
    result: Any = {"directory": directory, "can_write": False, "error": None}
...
def attempt_fix_nltk_installation(verbose: bool = False) -> Dict[str, Any]:
    ...
    """
    result: Any = {"nltk_installed": False, "vader_downloaded": False, "errors": []}
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if verbose:
                    print_success(
                        f"Successfully installed NLTK (version {nltk.__version__})"
                    )
            except ImportError as e:
                result["errors"].append(
                    f"Failed to import NLTK after installation: {str(e)}"
                )
        except Exception as e:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        result["errors"].append(
                            f"Failed to find VADER lexicon after download: {str(e)}"
                        )
                except Exception as e:
                    result["errors"].append(
                        f"Failed to download VADER lexicon: {str(e)}"
                    )
        except ImportError as e:
    ...
    if result["nltk_installed"] and not result["vader_downloaded"]:
        ...
        try:
            ...
            try:
                ...
                if verbose:
                    print_success(
                        f"Successfully downloaded VADER lexicon to {nltk_data_dir}"
                    )
            except LookupError as e:
                result["errors"].append(
                    f"Failed to find VADER lexicon after custom download: {str(e)}"
                )
        except Exception as e:
            result["errors"].append(
                f"Failed to download VADER lexicon to custom location: {str(e)}"
            )
...
def run_diagnostics(verbose: bool = False, fix: bool = False) -> Dict[str, Any]:
    ...
    for directory in nltk_data_dirs:
        ...
        results["nltk_data_dirs"].append(
            {"directory": directory, "permissions": dir_perms}
        )

        if dir_perms["exists"]:
            if dir_perms["readable"] and dir_perms["writable"]:
                print_success(
                    f"Directory exists with read/write permissions: {directory}"
                )
            elif dir_perms["readable"]:
                print_warning(
                    f"Directory exists with read-only permissions: {directory}"
                )
            else:
            ...
            if dir_perms["can_create"]:
                print_warning(
                    f"Directory does not exist but can be created: {directory}"
                )
            else:
                print_error(
                    f"Directory does not exist and cannot be created: {directory}"
                )
    ...
    if sia_result["can_analyze"]:
        ...
        print_error(
            f"SentimentIntensityAnalyzer initialized but analysis failed: {sia_result['error']}"
        )
    elif sia_result["can_import"]:
        print_error(
            f"SentimentIntensityAnalyzer import succeeded but initialization failed: {sia_result['error']}"
        )
    else:
    ...
    print_info(
        f"Modules using SentimentIntensityAnalyzer: {len(affected_modules['sentiment_analyzer'])}"
    )
...
def main():
    ...
    if nltk_ok and vader_ok and sia_ok:
        print_success(
            "All checks passed! NLTK and VADER lexicon are working correctly."
        )
        ...
        print_warning(
            "VADER lexicon is available but SentimentIntensityAnalyzer is not working."
        )
    ...
    if args.json:
        ...
        with open(args.output, "w") as f:
            ...
            f.write("\nAffected Modules:\n")
            f.write(
                f"  Using NLTK: {len(results['affected_modules']['nltk_import'])}\n"
            )
            f.write(
                f"  Using VADER: {len(results['affected_modules']['vader_lexicon'])}\n"
            )
            f.write(
                f"  Using SentimentIntensityAnalyzer: {len(results['affected_modules']['sentiment_analyzer'])}\n"
            )
