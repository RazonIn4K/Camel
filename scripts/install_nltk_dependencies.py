...
def download_nltk_data(
    package: str, download_dir: Optional[str] = None, quiet: bool = False
    ...
    try:
        ...
        if download_dir:
            try:
                ...
                try:
                    ...
                    if not quiet:
                        print_success(
                            f"Successfully downloaded '{package}' to {download_dir}"
                        )
                    return True, None
                except LookupError:
                    if not quiet:
        ...
        try:
            ...
            try:
                ...
                if not quiet:
                    print_success(
                        f"Successfully downloaded '{package}' to default location"
                    )
                return True, None
            except LookupError:
                if not quiet:
        ...
        if writable_dir:
            try:
                ...
                try:
                    ...
                    if not quiet:
                        print_success(
                            f"Successfully downloaded '{package}' to {writable_dir}"
                        )
                    return True, None
                except LookupError:
...
def manual_download_nltk_data(
    package: str, target_dir: str, quiet: bool = False
) -> Tuple[bool, Optional[str]]:
    """
...
def run_installation(
    ...
    for package in packages:
        ...
        if not quiet:
            print_warning(
                f"Standard download failed for {package}, trying manual download"
            )
        ...
        if success:
            ...
            if verify_nltk_data(package):
                ...
                if not quiet:
                    print_error(
                        f"Manual download succeeded but verification failed for {package}"
                    )
        else:
            results["nltk_data"][package] = {
                "installed": False,
    ...
    results["sentiment_analyzer"] = {"working": sia_works}
    ...
    if module_updates["updated_modules"]:
        if not quiet:
            print_success(
                f"Updated {len(module_updates['updated_modules'])} modules to use NLTK utilities"
            )
    ...
    if module_updates["errors"]:
        if not quiet:
            print_warning(
                f"Encountered {len(module_updates['errors'])} errors while updating modules"
            )
...
def main():
    ...
    parser.add_argument(
        "--data-dir", type=str, help="Specify custom NLTK data directory"
    )
    ...
    if not args.quiet:
        ...
        if nltk_ok and vader_ok and sia_ok:
            print_success(
                "Installation successful! NLTK and VADER lexicon are working correctly."
            )
            ...
            print_warning(
                "VADER lexicon is installed but SentimentIntensityAnalyzer is not working."
            )
    ...
    if args.json:
        ...
        with open(args.output, "w") as f:
            ...
            f.write("\nProject Updates:\n")
            f.write(
                f"  Updated requirements.txt: {results['dependency_updates']['requirements.txt']}\n"
            )
            f.write(
                f"  Updated setup.py: {results['dependency_updates']['setup.py']}\n"
            )
            f.write(f"  Created NLTK utilities: {results['nltk_utils']['created']}\n")
            f.write(
                f"  Updated modules: {len(results['module_updates']['updated_modules'])}\n"
            )
